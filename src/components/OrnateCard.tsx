    ...
    <div className={`relative group transition-all duration-700 ${className}`}>
      ...
      {title && (
        <div className="absolute -top-6 right-8 z-30">
          <div className="bg-black border-2 border-[#D4AF37] px-6 py-1 transform skew-x-12 md:glow-gold shadow-sm">
      ...
      {/* Main Container */}
      <div className={`
        relative overflow-hidden border-2 border-[#D4AF37] 
        
        /* OPTIMIZATION 1: No Blur on Mobile. Darker background for readability. */
        bg-black/95 md:bg-black/70 md:backdrop-blur-xl 
        
        p-6 md:p-8 
        
        /* OPTIMIZATION 2: Reduced Shadow on Mobile */
        shadow-lg md:shadow-[0_0_50px_rgba(212,175,55,0.15)] 
        
        transition-shadow duration-500 
        md:group-hover:shadow-[0_0_60px_rgba(212,175,55,0.25)]
      `}>
        
        {/* OPTIMIZATION 3: HIDE THE FAN ANIMATION ON MOBILE
            Generating 11 divs with gradients is too heavy for mobile scrolling.
        */}
        <div className="hidden md:flex absolute inset-0 items-end justify-center pointer-events-none overflow-hidden">
        ...
        {/* Ornate Corner Accents - Keeping these as they are static and cheap */}
        ...
        {/* Inner Border */}
        <div className="border border-[#D4AF37]/20 h-full relative z-10">
           {children}
        </div>
      </div>
      
      {/* Bottom Shadow Glow - Hidden on mobile */}
      <div className="hidden md:block absolute -bottom-4 left-1/2 -translate-x-1/2 w-[85%] h-6 bg-[#D4AF37]/15 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
    </div>
  );
};
