...
const Countdown: React.FC = () => {
  ...
  useGSAP(() => {
    ...
    // Animation logic is fine, keeping it
    gsap.to(tracker, {
  ...
  // OPTIMIZATION 1: Removed heavy glow on mobile. Added 'md:' to the heavy shadow.
  // Mobile gets a simple drop shadow (cheaper).
  const numberStyles = "text-6xl md:text-8xl font-black leading-none text-[#FFEBD0] w-[1.1em] text-center drop-shadow-md md:[text-shadow:0_4px_15px_rgba(0,0,0,0.9),0_0_25px_rgba(255,69,0,0.4)]";
  
  const labelStyles = "text-xl md:text-3xl tracking-widest text-[#FFEBD0]/80 mt-2 drop-shadow-sm md:[text-shadow:0_1px_5px_rgba(0,0,0,0.9)]";
  
  const separatorStyles = "text-4xl md:text-7xl text-[#FFEBD0]/80 mt-1 md:mt-0 mx-2 md:mx-4 drop-shadow-sm";

  return (
    <div
      className={`
        absolute z-40 select-none pointer-events-none flex items-start
        /* FIX FOR OVERLAP BELOW: */
        /* Old: top-[50%] (Centered) -> Causes overlap with Title */
        /* New: bottom-[15%] (Lower down) -> Clears the center space */
        bottom-[15%] left-1/2 -translate-x-1/2
        
        /* Desktop styles remain same (Bottom Left) */
