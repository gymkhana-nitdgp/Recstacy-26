...
const HamburgerMenu = ({
  ...
  useGSAP(
    () => {
      tl.current = gsap
        .timeline({ paused: isMd ? true : false, repeat: -1 })
        .to(chakraRef.current, {
          rotation: 360,
          duration: 8,
          ease: "none",
        });
      ...
      <div ref={container}>
        ...
        >
          {isMd ? (
            <div
              ref={chakraRef}
              className="top-2 left-2 w-10 h-10 absolute border-2 border-dashed border-zinc-400 rounded-full opacity-0"
            />
          ) : (
            <div
              ref={chakraRef}
              className="w-10 h-10 absolute border-2 border-dashed border-zinc-400 rounded-full "
            />
          )}
          <img
            src={LOGO}
            alt="logo image"
            className={`${isMd ? "h-14 hover:rotate-180 hover:scale-105 transition-transform duration-500 " : "h-10"} z-10 relative `}
          />
        </div>

        {/* Use isRendered instead of isOpen to keep the component alive during exit */}
        {isRendered && (
          <Modal modalRef={modalRef} setIsOpen={setIsOpen} handleNavClick={handleNavClick} />
        )}
      </div>
      {!isRendered && desktop && <NavMenus handleNavClick={handleNavClick} />}
    </>
  );
};
